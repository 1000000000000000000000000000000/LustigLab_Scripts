...
This script reformats the defline of FASTA files to allow dataset_prep_nf and
...
def main(file: Path | None = None, files: list[Path] | None = None, dir: Path = Path("."), chain: str = "A", inplace: bool = False) -> None:
	"""
	If 'file' is provided, process that single file.
	If 'files' is provided, process that specific list of files.
	Otherwise process all *.fasta in 'dir' non-recursively.

	Default behavior processes all FASTA files in the current working directory and writes *_modified.fasta copies.
	"""

	def is_valid_fasta(p: Path) -> bool:
		return p.exists() and p.is_file() and p.suffix.lower() == ".fasta"

	if file is not None:
		if not is_valid_fasta(file):
			print(f"ERROR: {file} not found or not a .fasta")
			return

		process_fasta(file, inplace=inplace, chain=chain)
		print("updated_format_fasta.py has finished running.")
		return

	if files is not None:
		valid_files = []
		for p in files:
			if not is_valid_fasta(p):
				print(f"WARNING: {p} not found or not a .fasta; skipped.")
				continue
			valid_files.append(p)

		if not valid_files:
			print("ERROR: No valid FASTA files were provided.")
			return
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(
		description="Reformats FASTA defline to look like the following: '>abcd_A'. "
		            "Default, with no args, processes all .fasta files in the current working directory "
		            "and writes new '*_modified.fasta' files."
	)

	input_group = parser.add_mutually_exclusive_group()
	input_group.add_argument("-f", "--file", type=Path, help="Single FASTA file to process")
	input_group.add_argument("--files", nargs="+", type=Path, help="Specific list of FASTA files to process")

	parser.add_argument("-d", "--dir", type=Path, default=Path("."), help="Directory with FASTA files, non-recursive")
	parser.add_argument("-c", "--chain", default="A", help="Chain letter to use in the reformatted defline, default: A")
	parser.add_argument("--inplace", action="store_true", help="Edit files in place, overwriting the originals")
